  ...
  class << self
    ...
    def SpamUpload http, auth_token, user_id, level
      ...
      begin
        ...
        puts "User #{user_id} upload Rescued: #{$!}"
		return "Rescued: #{$!}"
      end
	  sleep(0.1)
	  ...
	  return "Uploading spam #{level} to user #{user_id}"
    end
    ...
    def ProcessClean auth_token, user
      NetworkDo do |http|
	  puts "Username '#{user['username']}', level #{user['level']}".bg_green
        ...
        json['user_processes'].each do |p|
          if p['status'].to_i == 2 && (p['process_type_id'].to_i == 4 || p['process_type_id'].to_i == 3)
			#puts p.inspect
            puts "IP: ".magenta+p["ip"]+" Level: ".magenta+p["software_level"]+" Type: ".magenta+HackEx::Helper.SoftwareIdToName(p["software_type_id"].to_i)
			Request.Do(http, Request.ProcessDelete(auth_token, p['id']))
          end
		  if p['status'].to_i == 1 && p['software_type_id'].to_i == 2 
			#puts p.inspect
            puts "IP: ".magenta+p["ip"]+" Level: ".magenta+p["software_level"]+" Type: ".magenta+HackEx::Helper.SoftwareIdToName(p["software_type_id"].to_i)
			Request.Do(http, Request.ProcessDelete(auth_token, p['id']))
          end
